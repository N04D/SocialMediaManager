  ...
  */
  static schemaRules(schema) {
    ...
    for (let name in schema.marks) {
      let rules3 = schema.marks[name].spec.parseDOM;
      if (rules3)
        rules3.forEach((rule) => {
    ...
    for (let name in schema.nodes) {
      let rules3 = schema.nodes[name].spec.parseDOM;
      if (rules3)
        rules3.forEach((rule) => {
          insert(rule = copy(rule));
...
function inputRulesPlugin(props) {
  const { editor, rules: rules3 } = props;
...
var seed = window.__studioEditorSeed;
if (!seed) {
} else {
  ...
  let saveCounter = 0;
  let dragDepth = 0;
  const slugify = (value) => value.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
  ...
  const autosave = async () => {
    ...
    try {
      ...
      setLastSaved(data.updated_at || (/* @__PURE__ */ new Date()).toISOString());
  ...
  const initialContent = seed.editor_json && Array.isArray(seed.editor_json.content) && seed.editor_json.content.length ? seed.editor_json : seed.html_body && seed.html_body.trim() ? seed.html_body : "<p></p>";
  ...
  setCoverPreview(
    coverImagePathInput?.value?.trim() ? `/content-files/${coverImagePathInput.value.trim().replace(/^\.?\/*content\/drafts\//, "")}` : ""
  );
