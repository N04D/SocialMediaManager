...
class LinkedInConnectTests(unittest.TestCase):
    ...
    def test_connect_allows_remote_debugging_browser(self) -> None:
        ...
        with patch.object(
            ...
            return_value=("pw", None, None, None, False, "remote debugging session"),
        ) as opener:
    ...
    def test_non_interactive_browser_paths_disable_remote_debugging(self) -> None:
        self.config.linkedin_remote_debugging_url = "http://127.0.0.1:9222"
        with patch.object(
            browser_module, "open_linkedin_session", return_value=("pw", None, None, None, False, "persistent profile")
        ) as opener:
            browser_module.open_local_linkedin_session(
                self.config,
    ...
    def test_connect_status_moves_to_connected(self) -> None:
        ...
        context = FakeContext()
        runtime = fake_plugin_runtime_for_session(
            self.config, lambda *args, **kwargs: (playwright, None, context, page, True, "persistent profile")
        )
        with patch.object(connect_module, "get_plugin_runtime", return_value=runtime):
            with patch.object(
                connect_module,
                "inspect_linkedin_auth_state_session",
                return_value={
                    "authenticated": False,
                    "reason": "login",
                    "marker": "login_url",
                    "current_url": page.login_url,
                },
            ):
                with patch.object(connect_module, "wait_for_manual_linkedin_login_session", return_value=(True, "")):
                    result = connect_module.run_connect_action(
                        self.config,
                        channel_id="linkedin",
                        action_id=connection.active_job_id,
                        worker_id="worker-a",
                        started_at="2026-06-20T08:00:00+02:00",
                    )
    ...
    def test_connect_status_moves_to_needs_login_on_timeout(self) -> None:
        ...
        with patch.object(connect_module, "get_plugin_runtime", return_value=runtime):
            with patch.object(
                ...
                with patch.object(
                    connect_module,
                    "wait_for_manual_linkedin_login_session",
                    return_value=(False, "Timed out waiting for manual login."),
                ):
                    result = connect_module.run_connect_action(
                        self.config,
                        channel_id="linkedin",
    ...
    def test_profile_ownership_is_released_after_completion(self) -> None:
        ...
        with patch.object(connect_module, "get_plugin_runtime", return_value=runtime):
            with patch.object(
                connect_module,
                "inspect_linkedin_auth_state_session",
                return_value={"authenticated": True, "reason": "", "marker": "feed_url", "current_url": page.feed_url},
            ):
                result = connect_module.run_connect_action(
                    self.config,
                    channel_id="linkedin",
